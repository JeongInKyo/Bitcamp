...
- build : 컴파일된 .class 파일과 배포파일(.jar) 등이 생성된다.
...
//왼쪽 상단에 있는 확장에 들어간후 markdown 검색해서 Extensible Markdown Conveter(마크다운 미리보기) 다운받기

*****************
// gradle을 사용하려면 규칙에 맞게 그래들 관련 설정파일들이 존재해있어야된다.
예) c:\workspace\test> [gradle init --type java-application] 이 명령어 하나로 프로그램들이 다 생성된다.
*****************
...
### compileJava
자바 소스(src/main/java)를 컴파일하는 명령어이다. 소스 폴더의 파일을 컴파일하여 build/claases/java/main 폴더에 둔다.
...
### build
빌드는 모든 작업을 수행한다. 따라서 마지막으로 테스트 및 배포 파일을 생성할 때 실행한다.
c:\workspace\test> gradle build


### tasks
Gradle 명령으로 수행할수 있는 작업 정보를 알아낸다.
...
## Gradle 플러그인
build.gradle 파일에 등록된 플러그인을 등록해야만 그 플러그인의 task를 사용할 수 있다.
...
## wrapper
시나리오: 어떤 개발자가 프로젝트를 만든 후 다른 개발자에게 전달하려 한다.
         그런데 다른 개발자의 pc에 gradle가 깔려있지 않다.
         그렇다고 그 개발자가 gradle을 설치하는 번거로움을 주기 싫다.
         단지 실행하면 자동으로 gradle을 설치하게 하고 싶다.
         또는 자신이 사용한 gradle 버전을 다운로드 받게 하고 싶다.

         이런 경우를 위해 사용하는 것이 wrapper 이다.
         gradle에서는 가능한 wrapper 방식을 사용할 것을 권장하고 있다.
...
### Gladlew 사용 이유
예)회사에서 서버에 보낼때 서버에는 gradle가 안깔려있으니 gradle을 깔 필요 없이(컴파일도 할필요 없이) gradlew만으로 수행이 가능하기 때문.
...
3. User Manual 들어가면
4. 47. java 검색
5-1. 47.3 Tasks  들어간 후 살짝 아래로 내려서 표 보고 이해.
5-2. Wrapper task 들어가서 보고 이해.



트렌드가 Maven에서 Gradle로 넘어오는중.(회사에서는 아직 Maven을 쓰지만 모바일은 무조건 Gradle을 쓴다. 앞으로도 Gradle을 다 쓰게될것! 대세는 Gradle!)
앞으로 테스할때는 gradle classes로 컴파일하고 실행해서 테스트 할것.(선행)
문제없이 잘 실행되면 build로 실행.
...
해결 방법
1. git init
2. git remote add origin http://#@$^$%&@$#^/thsebr.git
3. git add
4. git commit -m "default"
5. git push -u origin master
git add .을 안하면 error:src refspec master does not match any.


내가 알아낸 방법(인터넷에서 검색해서 찾은방법)
git config --global push.default matching 등록 후 수정하고 esc : wq엔터 순서대로 누르면 된다.
